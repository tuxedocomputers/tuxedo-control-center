...
export const AquarisClientAPI =
    {
        connect: (deviceUUID: string): Promise<void> => ipcRenderer.invoke(aquarisAPIHandle, [AquarisAPIFunctions.connect, deviceUUID]),
        disconnect: (): Promise<void> => ipcRenderer.invoke(aquarisAPIHandle, [AquarisAPIFunctions.disconnect]),
        isConnected: (): Promise<boolean> => ipcRenderer.invoke(aquarisAPIHandle, [AquarisAPIFunctions.isConnected]) as Promise<boolean>,
        hasBluetooth: (): Promise<boolean> => ipcRenderer.invoke(aquarisAPIHandle, [AquarisAPIFunctions.hasBluetooth]) as Promise<boolean>,
        ...
        getDevices: (): Promise<DeviceInfo[]> => ipcRenderer.invoke(aquarisAPIHandle, [AquarisAPIFunctions.getDevices]) as Promise<DeviceInfo[]>,
        getState: (): Promise<AquarisState> => ipcRenderer.invoke(aquarisAPIHandle, [AquarisAPIFunctions.getState]) as Promise<AquarisState>,
        readFwVersion: (): Promise<string> => ipcRenderer.invoke(aquarisAPIHandle, [AquarisAPIFunctions.readFwVersion]) as Promise<string>,
        updateLED: (red: number, green: number, blue: number, state: RGBState | number): Promise<void> => ipcRenderer.invoke(aquarisAPIHandle, [AquarisAPIFunctions.updateLED, red, green, blue, state]),
        writeRGBOff: (): Promise<void> => ipcRenderer.invoke(aquarisAPIHandle, [AquarisAPIFunctions.writeRGBOff]),
        writeFanMode: (dutyCyclePercent: number): Promise<void> => ipcRenderer.invoke(aquarisAPIHandle, [AquarisAPIFunctions.writeFanMode, dutyCyclePercent]),
        writeFanOff: (): Promise<void> => ipcRenderer.invoke(aquarisAPIHandle, [AquarisAPIFunctions.writeFanOff]),
        writePumpMode: (dutyCyclePercent: number, voltage: PumpVoltage | number): Promise<void> => ipcRenderer.invoke(aquarisAPIHandle, [AquarisAPIFunctions.writePumpMode, dutyCyclePercent, voltage]),
